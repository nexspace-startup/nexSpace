...
const NavbarPanel: React.FC<NavbarPanelProps> = ({ items, activeId, onSelect }) => {
  return (
    <>
      ...
      <nav
        className="hidden md:flex w-[76px] shrink-0 h-screen bg-[#131316] border-r border-[#26272B] flex-col items-center py-5 gap-10"
      >
        {/* Logo */}
      ...
      {/* Mobile bottom nav */}
      <nav className="md:hidden fixed bottom-0 inset-x-0 z-30 h-14 bg-[#131316] border-t border-[#26272B] grid grid-cols-4">
        {items?.slice(0, 4).map((n) => (
          <button
            key={n.id}
            onClick={() => onSelect?.(n.id)}
            className={["flex items-center justify-center", activeId === n.id ? "bg-[#202024]" : ""].join(" ")}
            aria-label={n.label}
          >
            <span className="w-6 h-6 text-[#80889B]">
              {n.icon ?? (
